...
<div class="container-fluid p-4">
    ...
    <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between mb-4 gap-3">
        <div>
            <h4 class="fw-800 text-dark mb-1">Manage Certificates</h4>
            <div class="d-flex align-items-center gap-2">
                <span class="badge bg-soft-primary text-primary px-3 py-2 rounded-pill">
                    <i class="fa-solid fa-file-contract me-1"></i> {{ $certificates->total() }} Total Requests
                </span>
                <span class="text-muted small">• Review attendance and process approvals</span>
            </div>
        </div>
    </div>
        ...
        <div class="grid-body">
            @forelse($certificates as $key => $cert)
                <div class="grid-row animate__animated animate__fadeInUp" style="animation-delay: {{ $key * 0.05 }}s">
                    ...
                    {{-- Column 4: Remarks --}}
                    <div>
                        <span class="text-muted small text-truncate d-inline-block" style="max-width: 180px;" title="{{ $cert->faculty_remarks }}">
                            <i class="fa-regular fa-comment-dots me-1"></i>
                            {{ $cert->faculty_remarks ?? 'No remarks provided' }}
                        </span>
                    </div>

                    {{-- Column 5: Actions (Approval Form or Status) --}}
                    <div class="d-flex justify-content-lg-end pe-2 w-100">
                        @if($cert->status === 'faculty_approved')
                            <form action="{{ route('admin.certificates.process', $cert) }}" method="POST" class="d-flex flex-column flex-sm-row gap-2 w-100 align-items-sm-center justify-content-sm-end form-approval-wrapper">
                                @csrf
                                ...
                                {{-- Remarks Input --}}
                                <input type="text" name="remarks" placeholder="Remarks (if rejecting)" class="form-control form-control-sm pro-input shadow-none mb-0 py-1" style="max-width: 180px; border-radius: 8px;">
                        ...
                        @else
                            <span class="badge bg-light text-dark border px-3 py-2 rounded-pill text-capitalize shadow-sm">
            ...
            @empty
                <div class="p-5 text-center w-100">
                    <div class="display-1 text-muted opacity-25 mb-3"><i class="fa-solid fa-file-contract"></i></div>
                    <h5 class="fw-bold text-dark">No certificate requests</h5>
                    <p class="small text-muted mb-0">There are currently no certificates requiring your attention.</p>
                </div>
            @endforelse
        </div>
    </div>
...
<style>
    /* Give the final column more room for the form elements */
    @media (min-width: 992px) {
        .grid-header, .grid-row { 
            grid-template-columns: 1.5fr 1.5fr 1fr 1.5fr 2.5fr; 
        }
    }
    
    /* Ensure the inline form inputs don't look weird on mobile when stacked */
